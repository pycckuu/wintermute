...
- [ ] 1. Add OpenAI auth type and resolver in `src/credentials.rs`
- [ ] 2. Add `src/providers/openai.rs` implementing `LlmProvider` via OpenAI Chat Completions API with tool-calling support
- [ ] 3. Export `openai` module in `src/providers/mod.rs`
- [ ] 4. Wire `openai` branch in `src/providers/router.rs` with clear missing-credential errors
- [ ] 5. Update startup wiring in `src/main.rs` to resolve/register OpenAI auth secrets and add provider-aware auth logging
- [ ] 6. Update bootstrap `.env` template to include `OPENAI_API_KEY` and `OPENAI_OAUTH_TOKEN`
- [ ] 7. Update `config.example.toml` and relevant docs with `openai/gpt-*` examples
- [ ] 8. Add tests for credentials resolution priority and secret exposure behavior
- [ ] 9. Add OpenAI provider wire-format tests (`tests/providers/openai_test.rs`)
- [ ] 10. Extend provider contract/router tests for OpenAI model specs
- [ ] 11. Run `cargo fmt --all`, `cargo clippy --all-targets --all-features -- -D warnings`, and targeted/full test suites
